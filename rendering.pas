procedure draw_line(dist: Real; posx, posy: integer; angle: Real; anglez: real);
           var angleZ1,
               color_draw2: integer;
         ...
         begin
           ...
           SDL_RenderDrawLine(rend, 3*loop, halfheight-draw_yoffset_up+anglez1,3*loop, halfheight+draw_yoffset_down+anglez1);     //draw a linie
           SDL_RenderDrawLine(rend, 3*loop+1, halfheight-draw_yoffset_up+anglez1,3*loop+1, halfheight+draw_yoffset_down+anglez1); //draw a linie
           SDL_RenderDrawLine(rend, 3*loop+2, halfheight-draw_yoffset_up+anglez1,3*loop+2, halfheight+draw_yoffset_down+anglez1); //draw a linie
           end;

procedure DrawBackground(bgimg: pSDL_Texture;zangle: real) ;
         var draw_gnd_rect: tSDL_Rect;                            //not a pointertype now
         begin
               //new(draw_gnd_rect);                              //memory leak! NEW without DISPOSE!
               draw_gnd_rect.x:=0;
               draw_gnd_rect.y:=TRUNC(zangle)*3-269;
