	...
	begin
     ...
     begin
	  ...
	  writeln('Y: ', pl_y:0:2);
          ...
          if map[trunc((pl_x + distray_x(player_mov,rotate))/map_scale),trunc(pl_y/map_scale)] = 0 then
          begin
            pl_x := pl_x + distray_x(player_mov,rotate);
          end;
          if map[trunc(pl_x/map_scale),trunc((pl_y + distray_y(player_mov,rotate))/map_scale)] = 0 then
          begin
...
//          writeln('Y: ', pl_x);
          ...
          if map[trunc((pl_x + distray_x(player_mov,rotate+90))/map_scale),trunc(pl_y/map_scale)] = 0 then
          begin
            pl_x := pl_x + distray_x(player_mov,rotate+90);
          end;
          if map[trunc(pl_x/map_scale),trunc((pl_y + distray_y(player_mov,rotate+90))/map_scale)] = 0 then
          begin
...
//          writeln('Y: ', pl_x);
     ...
     rotate := rotate+trunc((mouswidth-mouse_x)/3);                      //rotate horizontal;  rotate players view / direction
     rotatez := rotatez+trunc((mousheight-mouse_y)/2);                   //rotate vertical;    looking up or down
