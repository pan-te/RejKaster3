...
begin
  ...
  begin
    {if(map[round((pl_x + distray_x(9,rotate))/16), round((pl_y + distray_y(9,rotate))/16)] = 0) then begin
     ...
     if map[round((pl_x + distray_x(player_mov, rotate)) / map_scale), round(pl_y / map_scale)] = 0 then begin
       pl_x := pl_x + distray_x(player_mov, rotate);
     end;
     if map[round(pl_x / map_scale), round((pl_y + distray_y(player_mov, rotate)) / map_scale)] = 0 then begin
       pl_y := pl_y + distray_y(player_mov, rotate);
     end;
     Set_Movebob();
  end;
  ...
  begin
    ...
    if map[round((pl_x + distray_x(player_mov,rotate+90))/map_scale),round(pl_y/map_scale)] = 0 then begin
      pl_x := pl_x + distray_x(player_mov,rotate+90);
    end;
    if map[round(pl_x/map_scale),round((pl_y + distray_y(player_mov,rotate+90))/map_scale)] = 0 then begin
